...
# 1. إعدادات الصفحة وتنسيق الواجهة (CSS) للوضوح الأقصى
st.set_page_config(page_title="DOGGA SYSTEM | م/ ياسين علاء", layout="wide")

st.markdown("""
    <style>
    /* تنسيق الجداول: خط كبير، عريض، وممركز */
    .stTable td { 
...
# --- صفحة التطبيق الرئيسي ---
        ...
        with c3:
            ...
            if st.button("✅ تحسيب وحفظ"):
                if w and h:
                    ...
                    # بناء جدول الألومنيوم بـ 5 أعمدة مفصلة
                    alum_data = [
                    ...
                    if sh_n > 0:
                        alum_data.append({"البند": "عوارض رف (عرض)", "مقاس المفرد": int(sh_w), "عدد المفرد": f"{sh_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                        alum_data.append({"البند": "عوارض رف (عمق)", "مقاس المفرد": int(sh_d), "عدد المفرد": f"{sh_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                    if dv_n > 0:
                        alum_data.append({"البند": "عوارض فاصل (ارتفاع)", "مقاس المفرد": int(dv_h), "عدد المفرد": f"{dv_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                        alum_data.append({"البند": "عوارض فاصل (عمق)", "مقاس المفرد": int(dv_d), "عدد المفرد": f"{dv_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                    if dr_n > 0:
                        alum_data.append({"البند": "إطار درج (عرض-2.5)", "مقاس المفرد": int(dr_w-2.5), "عدد المفرد": f"{dr_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                        alum_data.append({"البند": "إطار درج (عمق)", "مقاس المفرد": int(dr_d), "عدد المفرد": f"{dr_n*2} ق", "مقاس المتقارب": "-", "عدد المتقارب": "-"})
                    ...
                    if sh_n > 0: fiber_data.append({"القطعة": "فيبر رف", "المقاس الصافي": f"{int(sh_w-5)} x {int(sh_d-5)}", "العدد": int(sh_n)})
                    if dv_n > 0: fiber_data.append({"القطعة": "فيبر فاصل", "المقاس الصافي": f"{int(dv_h-5)} x {int(dv_d-5)}", "العدد": int(dv_n)})

                    # حساب الجرد للسايد بار
                    ...
                    st.session_state.project_list.append({
                        "name": u_name, "dims": f"{w}x{h}x{d}", "alum": pd.DataFrame(alum_data),
                        "fiber": pd.DataFrame(fiber_data), "m_m": m_m, "m_t": m_t, "f_a": f_area, "notes": u_notes
                    })
                    st.rerun()
    ...
    for idx, item in enumerate(st.session_state.project_list):
        ...
        st.table(item['alum']) # الجدول المقسم بوضوح

...
# --- 2. واجهة إدخال البيانات والنتائج ---
    ...
    with st.container():
        ...
        if st.button("💾 احسب المقاسات"):
            if w and h and d:
                # قوانين التخصيم الأساسية
                h_bak = h - 13 if u_type in ["سفلية", "دولاب خزين"] else h - 5
                w_bak, d_bak = w - 5, d - 5
                
                # تخصيم الرفوف والفواصل (خصم 5 سم من مقاس الباكية كما طلبت)
    ...
    # --- عرض النتائج بالصيغة اللي بتحبها ---
    if st.session_state.storage:
        st.markdown("<br><h3 style='text-align: right;'>📋 نتائج التخصيم:</h3>", unsafe_allow_html=True)

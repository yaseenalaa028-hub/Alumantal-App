...
# 1. إعدادات الصفحة والستايل الاحترافي (حل مشكلة وضوح الكلام)
...
st.markdown("""
    ...
    .stTable td { 
        ...
        white-space: pre-line !important; /* للسماح بنزول الكلام في سطر جديد */
    }
    .stTable th { background-color: #2c3e50 !important; color: white !important; font-size: 18px !important; }
    ...
    .unit-card { background-color: #ffffff; padding: 20px; border-radius: 15px; border-right: 12px solid #2c3e50; margin-bottom: 30px; box-shadow: 0px 5px 15px rgba(0,0,0,0.1); }
    ...
    with st.expander("➕ إضافة وحدة جديدة (أدخل البيانات هنا)", expanded=True):
        ...
        with c3:
            ...
            if st.button("✅ تحسيب وحفظ الوحدة"):
                if w and h:
                    # 1. معادلات التخصيم الأساسية (إلغاء الأصفار باستخدام int)
                    h_b = int(h - 13) if u_type in ["سفلية", "دولاب خزين"] else int(h - 5)
                    w_b, d_b = int(w - 5), int(d - 5)
                    
                    # 2. تحديد نوع الخصام (فصل في أسطر)
                    if u_type == "سفلية":
                        type_h, type_w, type_d = "2 مفرد\n2 متقارب", "3 مفرد\n1 متقارب", "2 مفرد\n2 متقارب"
                    else:
                        type_h, type_w, type_d = "2 مفرد\n2 متقارب", "2 مفرد\n2 متقارب", "4 متقارب"

                    # 3. بناء جدول الألومنيوم
                    alum_data = [
                    ...
                    # 4. بناء جدول الفيبر (إلغاء الأصفار)
                    fiber_data = [
